...
def register():
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(sql, (id, hashed_pw, nickname))
        conn.commit()
        return render_template('index.html', success="회원가입 완료")
    
    except Exception as e:
        return jsonify({"result":"fail", "error":"이미 존재하는 아이디입니다"})
...
def reset_pw():
    ...
    return render_template('reset-pw.html', user_id=user_id)
...
def find_id():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT user_id FROM users WHERE nickname = %s AND school = %s",
            (nickname, school)
        )
        result = cursor.fetchone()
        conn.close()
        ...
        if result:
            ...
            return render_template('find_id.html', error="정보가 일치하지 않습니다")
...
def mypage(user_id):
    ...
    if 'user_id' not in session:
        return jsonify({"result":"fail", "message":"로그인 후 가능합니다"})
...
@app.route('/users/<string:user_id>/update', methods=['PUT'])
def update_mypage(user_id):
    ...
    if file:
        profile_img_name = secure_filename(file.filename)
    ...
    try:
        with conn.cursor() as cursor:
            if profile_img_name:
                ...
                cursor.execute(
                "UPDATE users SET nickname=%s, bio=%s, school=%s, profile_img=%s WHERE user_id=%s",(nickname, bio, school, profile_img_name, session["user_id"]))
            else:
                cursor.execute(
                    "UPDATE users SET nickname=%s, bio=%s, school=%s WHERE user_id=%s",
                    (nickname, bio, school, session['user_id'])
                )
        conn.commit()
